...
class _ReviewClipScreenState extends State<ReviewClipScreen> {
  ...
  Future<void> _loadInitial() async {
    ...
    try {
      ...
      final clip = await _api.fetchNextClip();
      if (!mounted) return;
      setState(() {
        _summary = summary;
        _clip = clip;
  ...
  Future<void> _saveAndNext() async {
    ...
    try {
      ...
      final summary = await _api.fetchSummary();
      final next = await _api.fetchNextClip();
      if (!mounted) return;
      setState(() {
        _summary = summary;
        _clip = next;
  ...
  void _resetReviewForm(ReviewClip? clip) {
    _notesController.clear();
    _rating = (clip?.reviewRating?.round() ?? 5).clamp(1, 5);
    _reason = clip?.reviewReason.isNotEmpty == true
        ? clip!.reviewReason
        : 'bom';
    _status = 'approved';
  }
    ...
    return Scaffold(
      body: SafeArea(
        child: Container(
          ...
          child: Column(
            children: [
              _Header(summary: _summary, onRefresh: _loadInitial),
  ...
  Widget _body() {
    ...
    if (clip == null) {
      return _CenteredMessage(
        ...
        detail: 'Todos os exports foram revisados.',
        onRetry: _loadInitial,
      );
    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          _ProgressHint(summary: _summary),
          const SizedBox(height: 12),
          ClipVideoPlayer(url: videoUrl),
          const SizedBox(height: 12),
          _ClipInfoCard(clip: clip),
          const SizedBox(height: 12),
          RatingStars(
            value: _rating,
...
class _ProgressHint extends StatelessWidget {
  const _ProgressHint({required this.summary});
...
class _ClipInfoCard extends StatelessWidget {
  const _ClipInfoCard({required this.clip});
